---
layout: post
title: Reinforcement Learning (RL) — From Fundamentals to PPO & GRPO in LLMs (II)
subtitle:
...
1. **Policy Model (π<sub>θ</sub> / Actor)**  
...
2. **Old Policy Model (π<sub>θₒₗd</sub> / Reference Policy)**  
...
3. **Value Model (V<sub>ϕ</sub> / Critic)**  
...
🧠 **LLM Example:**  
In RLHF training:
- **Actor (π<sub>θ</sub>):** GPT-like model generating completions.  
- **Old Policy (π<sub>old</sub>):** frozen copy used for rollout generation.  
- **Critic (V<sub>ϕ</sub>):** small value head attached to the model, predicting scalar “expected reward.”  
...
3. Perform several gradient updates (3–10 epochs).  
...
2. **Generate trajectories (prompts → completions):**  
...
5. **Value targets (for critic loss):**  
   Build bootstrapped returns $ R^{\text{target}}_t $ (e.g., from $ \hat{A}_t + V_t $ or n-step returns) for the value loss.
...
| **KL Constraint** | Trust region vs. π<sub>old</sub> | KL penalty vs. π<sub>ref</sub> | Keeps policy aligned with base model. |
...
4. **Group Advantage Calculation:** Normalize rewards → compute $ A = \frac{R - \text{mean}(R)}{\text{STD}(R)} $.  
5. **Policy Update:**  
