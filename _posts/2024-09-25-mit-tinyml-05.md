...
| **Signed Integer**      | Represents positive and negative whole numbers.                                                   | **Sign-Magnitude**: 8-bit `00000001` is 1, `10000001` is -1. **Two’s Complement**: 8-bit `00000001` is 1, `11111111` is -1.                    | \[-2<sup>n-1</sup>, 2<sup>n-1</sup> - 1\]                   | Two’s complement is standard due to the absence of duplicate zeros.                       |
...
📊 Linear quantization involves mapping floating-point numbers to integers for efficient computation and storage
...
### 4. ⚙️ Inference with Linear Quantization
...
### 🔑 Key Advantages of Linear Quantization
- **⚡ Faster computation** – Integer arithmetic is faster and more energy-efficient than floating-point.  
- **💾 Reduced memory** – Integer weights use less space.  
- **🛠️ Hardware acceleration** – Modern platforms optimize for integer operations.  
...
3. **Challenges in Real-World Frameworks:**
   - **PyTorch and current hardware** don’t fully support **integer-only operations** (like integer matrix multiplications) on quantized tensors.
